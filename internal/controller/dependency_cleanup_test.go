...
func TestDNSReconcilerPersistsStatusRouterWhenSelectionIsAmbiguous(t *testing.T) {
	scheme, objects, factory, clients := externalCleanupFixture(t)
	record := api.MikroTikDNSRecord{
		ObjectMeta: metav1.ObjectMeta{
			Name:       "dns",
	...
	for name, routerClient := range clients {
		if routerClient.deletedDNS != 0 {
			t.Fatalf("%s was cleaned before the durable target was recorded: DNS=%d", name, routerClient.deletedDNS)
		}
	}
	var stored api.MikroTikDNSRecord
...
func (client *recordingRouterClient) EnsureRouteWithDistance(_ context.Context, destination, gateway string, distance int32, _ string) error {
	client.ensuredRouteDestinations = append(client.ensuredRouteDestinations, destination)
	client.ensuredRouteGateways = append(client.ensuredRouteGateways, gateway)
	client.ensuredRouteDistances = append(client.ensuredRouteDistances, distance)
	return nil
}
